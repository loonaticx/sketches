...
# If you set this true, the screen will flash with textures drawn in a special mode that shows the mipmap detail level
# and texture size for each texture.
...
class driver(ShowBase):
    def __init__(self):
        ...
        # If we run this at runtime, the texture memory viewer won't pick up the 3 initial textures.
        # It will only display the overriding texture.
    ...
    def modifyTexture2048(self):
        ...
        """
        self.modelDemo.findAllMatches('**/ring_top').setTexture(loader.loadTexture('demo_model/texture2048.png'), 1)

    def modifyTexture4096(self):
        """
        4096 x 4096 texture, will take up 48.0 MB of texture memory. (Uncompressed)
        """
        self.modelDemo.findAllMatches('**/ring_top').setTexture(loader.loadTexture('demo_model/texture4096.png'), 1)
